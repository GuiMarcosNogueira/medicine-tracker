  ...
  return (
    <View style={s.wrapper}>
      ...
      >
        ...
        <Ionicons name={open ? 'calendar' : 'calendar-outline'} size={18} color={open ? theme.primary : theme.textMuted} />
      </Pressable>
        ...
        <>
          ...
          <View style={s.dropdown}>
            ...
            <View style={s.navRow}>
              ...
              <Text style={s.monthTitle}>{MONTHS_PT[viewMonth]} {viewYear}</Text>
            ...
            <View style={s.dowRow}>
              {DAYS_PT.map(d => (
                <Text key={d} style={[s.cell, s.dowCell]}>{d}</Text>
              ))}
            </View>
            ...
            <View style={s.grid}>
              {grid.map((day, idx) => {
                ...
                return (
                  <Pressable
                    key={idx}
                    style={[s.cell, s.dayCell, isSelected && s.dayCellSelected, !isSelected && isToday && s.dayCellToday]}
                    onPress={() => { onChange(dateToISO(new Date(viewYear, viewMonth, day))); setOpen(false); }}
                  >
                    <Text style={[s.dayText, isSelected && s.dayTextSelected, !isSelected && isToday && s.dayTextToday]}>
                      {day}
                    </Text>
                  </Pressable>
                );
...
function styles(t: Theme) {
  return StyleSheet.create({
    ...
    dropdown:   {
      ...
      shadowOpacity: t.isDark ? 0.4 : 0.1,
      shadowRadius: 16,
      elevation: 8,
    },
    ...
    navBtn:     { padding: 4 },
    monthTitle: { fontSize: 15, fontWeight: '700', color: t.text, fontFamily: fonts.heading },

    dowRow:     { flexDirection: 'row' },
    dowCell:    { fontSize: 10, color: t.textMuted, fontFamily: fonts.mono, textAlign: 'center' as never, paddingVertical: 4 },
    ...
    dayText:         { fontSize: 13, color: t.text, fontFamily: fonts.body },
